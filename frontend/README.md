...
Each builds to **one self-contained file** with all JS/CSS inlined:
    ...
    src/ctrlrunner/reporting/_static/report/index.html
    src/ctrlrunner/ui/_static/ui/ui.html

Both files are **committed to git** so the Python package and wheel need
no Node toolchain. `render_html()` (src/ctrlrunner/reporting/html_report.py)
replaces the report page's `<!--CTRLRUNNER_DATA-->` marker with the
`window.__CTRLRUNNER_REPORT__ = {...}` payload; `render_ui_html()`
(src/ctrlrunner/ui/ui_frontend.py) replaces the `__CTRLRUNNER_SESSION_TOKEN__`
...
npm run dev:ui     # UI Mode dev server; proxies /api to a running
                   #   `python -m ctrlrunner ui` (set CTRLRUNNER_UI_PORT, pass ?token=)
...
(`src/ctrlrunner/reporting/html_report.py::_result_to_dict`); keep
