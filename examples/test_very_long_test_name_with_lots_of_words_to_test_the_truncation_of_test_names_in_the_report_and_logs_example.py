...
# very_long_test_name_with_lots_of_words_to_test_the_truncation_of_test_names_in_the_report_and_logs

@test(timeout=15, case_id="TC-EX-001", tags={"smoke", "example", "demo", "test", "playwright", "ctrlrunner", "python"})
def test_example_dot_com(page):
    page.goto("https://example.com")
    assert page.title() == "Example Domain"


@test(timeout=15, case_id="TC-EX-002", tags={"smoke", "team_1"})
def test_playwright_dev(page):
    page.goto("https://playwright.dev")
    expect(page).to_have_title(re.compile("Playwrightttt"))


@test(timeout=15, case_id="TC-EX-003", tags={"smoke", "team_1"})
def test_playwright_dev_url(page):
    page.goto("https://playwright.dev")
    expect(page).to_have_url("https://playwright.dev/")


@test(timeout=15, case_id="TC-EX-004", tags={"smoke", "team_1"})
def test_playwright_dev_get_started_visible(page):
    page.goto("https://playwright.dev")
    expect(page.get_by_role("link", name="Get started")).to_be_visible()


@test(timeout=15, case_id="TC-EX-005", tags={"smoke", "team_2"})
def test_playwright_dev_heading_text(page):
    page.goto("https://playwright.dev")
    expect(page.locator("h1")).to_contain_text("Playwrightttt")


@test(timeout=15, case_id="TC-EX-006", tags={"smoke", "team_2"})
