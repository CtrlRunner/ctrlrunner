"""
Optional ctrlrunner.toml config file. CLI flags always take precedence over
it; it just sets defaults so CI/dev don't need to repeat the same flags.
    ...
    # class -- note the nesting: [ctrlrunner.workers], not a bare [workers]
    ...
    # -- populated by a human after reviewing `ctrlrunner flaky-report` output
    ...
    #    (omit, include, branch, ...); ctrlrunner only ever sets data_file/
...
never an error, so configs written for other ctrlrunner versions still
load.
"""
...
if TYPE_CHECKING:
    ...
    # in case ctrlrunner ever runs under an older interpreter.
    import tomllib
else:
    try:
...
# Every scalar key ever read out of the [ctrlrunner] table (unknown
...
# Nested tables under [ctrlrunner.<name>]. A bare top-level [<name>] is
...
def load_config(path: str, warn=None) -> dict:
    ...
    # that a newer/older ctrlrunner wrote, it just should not be silent.
    warn = warn or _default_warn
    for key in data:
        if key == "ctrlrunner":
            continue
        hint = f" -- did you mean [ctrlrunner.{key}]?" if key in KNOWN_TABLES else ""
        warn(f"top-level {key!r} in {path} is ignored{hint}")
    section = data.get("ctrlrunner", {})
