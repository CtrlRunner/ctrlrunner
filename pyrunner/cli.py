...
def _resolve_worker_settings(args, config):
    ...
    the [pyrunner.workers] constraint table and the fully_parallel
    ...
    try:
        ...
        print(f"Error: invalid [pyrunner.workers] config: {e}", file=sys.stderr)
        sys.exit(1)
...
def _resolve_bind_host(bind: str, allow_remote: bool) -> str:
    ...
    check plus a per-session token (see pyrunner/ui/localsec.py) -- that's
...
def _show_report(argv):
    parser = argparse.ArgumentParser(prog="pyrunner show-report")
    parser.add_argument(
        "path",
        nargs="?",
...
def _ui(argv):
    parser = argparse.ArgumentParser(prog="pyrunner ui")
    ...
    parser.add_argument("--config", default="pyrunner.toml")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "overriding a truthy pyrunner.toml 'headed' (default: headless)",
    )
    args = parser.parse_args(argv)
    ...
    try:
        ...
        print(f"Error: invalid [pyrunner.quarantine] config: {e}", file=sys.stderr)
        sys.exit(1)
    bind = _resolve_bind_host(args.bind, args.allow_remote)
    serve_ui(
        root,
...
def _flaky_report(argv):
    parser = argparse.ArgumentParser(prog="pyrunner flaky-report")
    parser.add_argument(
        "--window",
        type=int,
        default=None,
        help="How many recent runs per test to consider (default: config's "
        "[pyrunner.history].window_runs, or 20)",
    )
    ...
    try:
        ...
        print(f"Error: invalid [pyrunner.history] config: {e}", file=sys.stderr)
        sys.exit(1)
    if not history_config.enabled:
        print(
            "Error: [pyrunner.history] is disabled -- flaky-report needs history data.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(prog="pyrunner")
    parser.add_argument("--version", action="version", version=f"pyrunner {__version__}")
    parser.add_argument(
        "root",
        nargs="?",
        default=None,
        help="Directory to discover test_*.py files in, or a single test file "
        "(falls back to pyrunner.toml's 'root', then 'tests')",
    )
    parser.add_argument("-n", "--num-workers", type=_num_workers_arg, default=None)
    ...
    parser.add_argument(
        ...
        "explicitly forces it off, overriding a truthy pyrunner.toml 'report_timestamp'",
    )
    parser.add_argument(
        "--keep-reports",
        type=int,
    ...
    parser.add_argument(
        "--full-trace",
        action="store_true",
        help="Show full failure tracebacks including pyrunner-internal frames "
    ...
    parser.add_argument(
        ...
        "(requires the 'coverage' extra: pip install pyrunner[coverage])",
    )
    parser.add_argument(
        "--coverage-html",
        nargs="?",
    ...
    parser.add_argument(
        ...
        default=None,
        help="Browser for pyrunner.playwright_fixtures (default: chromium)",
    )
    parser.add_argument(
        "--headed",
        action=argparse.BooleanOptionalAction,
    ...
    parser.add_argument(
        "--project",
        default=None,
        help="Comma-separated project names to run (see [pyrunner.projects.*] "
    ...
    parser.add_argument(
        ...
        "registered_tags, overriding pyrunner.toml's strict_tags for this run",
    )
    ...
    if order == "random":
        if seed is None:
            ...
            print(f"pyrunner: --order random with no --seed -- using seed {seed}", file=sys.stderr)
    ...
    if requested_projects and tag_registry is not None:
        for _name in requested_projects:
            ...
            if _unregistered:
                _message = (
                    f"[pyrunner.projects.{_name}] tags: "
    ...
    if rerun_requested:
        ...
        # --project's run_projects() already handles -- if pyrunner's
    ...
    try:
        ...
        print(f"Error: invalid [pyrunner.log_redaction] config: {e}", file=sys.stderr)
        sys.exit(1)
    browser_name = args.browser or config.get("browser", "chromium")
    headed = args.headed if args.headed is not None else config.get("headed", False)
    playwright_config = {
    ...
    try:
        ...
        print(f"Error: invalid [pyrunner.coverage] config: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    if history_config.enabled:
        ...
        # root; [pyrunner.history].db_path overrides this explicitly.
        history_db_path = history_config.db_path or str(Path(reports_dir) / ".history.db")
    ...
    try:
        ...
        print(f"Error: invalid [pyrunner.fail_policy] config: {e}", file=sys.stderr)
        sys.exit(1)
    try:
        quarantine_config = resolve_quarantine_config(config)
    except ValueError as e:
    ...
    if html_report_path:
        ...
        print(
            ...
            f"(run 'python -m pyrunner show-report {report_path}' to view it)"
        )
    else:
        print(f"Reports written to {report_dir}")
