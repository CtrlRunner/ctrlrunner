...
CATEGORY_LABELS = {
    ...
    "playwright": "pytest-playwright fixtures -> pyrunner.playwright.playwright_fixtures",
    "imports": "import lines added/removed",
    "config": "pyproject.toml options -> pyrunner.toml",
}
...
class MigrationReport:
    ...
    def render_console(self, wrote: bool) -> str:
        ...
        if self.total_todos:
            ...
            lines.append(
                ...
                f"marked with '# TODO(pyrunner-migrate)' in the code:"
            )
            for f in self.files:
    ...
    def render_markdown(self, wrote: bool) -> str:
        totals = self.totals()
        lines = ["# pytest -> pyrunner migration report", ""]
