...
def _require_libcst():
    if cst is None:
        raise SystemExit(
            "pyrunner.migrate requires libcst (needed only for migration, "
...
def _import_module_name(stmt) -> str | None:
    """Module name for single-import lines ('pytest', 'pyrunner', ...)."""
...
def fix_imports(
    ...
    for stmt in body:
        ...
        if isinstance(imp, cst.Import):
            ...
            for alias in imp.names:
                ...
                # Names re-imported from pyrunner shadow-replace the
    ...
    if kept_pytest:
        report.todo(
            ...
            "marked with TODO(pyrunner-migrate) above",
        )
    body = new_body

    # --- merge into an existing `from pyrunner import ...` if present
    still_needed = set(needed)
    for i, stmt in enumerate(body):
        if (
            _import_module_name(stmt) == "pyrunner"
    ...
    inserts = []
    if still_needed:
        inserts.append(f"from pyrunner import {', '.join(sorted(still_needed))}\n")
    existing_code = cst.Module(body=body).code
    playwright_missing = sorted(
        n
        for n in playwright
        if not re.search(
            rf"from\s+pyrunner\.playwright\.playwright_fixtures\s+import\s+[^\n]*\b{n}\b",
            existing_code,
        )
    )
