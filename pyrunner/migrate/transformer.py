...
What gets converted automatically:
    ...
    pytest-playwright page/context/browser
                                        -> from pyrunner.playwright.playwright_fixtures import ...

Everything else gets '# TODO(pyrunner-migrate): ...' at the exact spot
...
TODO_PREFIX = "# TODO(pyrunner-migrate): "
...
# pytest builtin fixtures with no pyrunner equivalent.
...
# pytest metadata fixtures with direct pyrunner equivalents -- runtime
...
# variant is also RENAMED at its call sites, pytest name -> pyrunner
# name).
PROPERTY_FIXTURES = {
...
# pytest-playwright fixtures that map 1:1 onto pyrunner.playwright.playwright_fixtures.
...
MANUAL_PYTEST_ATTRS = {
    "raises": "pytest.raises has no pyrunner equivalent -- rewrite as "
    "try/except + assert (or plain 'with' helper of your own)",
    "warns": "pytest.warns has no pyrunner equivalent -- use warnings.catch_warnings",
    "approx": "pytest.approx has no pyrunner equivalent -- use math.isclose / abs(a-b) < eps",
    ...
    "exit": "pytest.exit has no pyrunner equivalent",
    "main": "pytest.main -- run 'python -m pyrunner' instead",
...
class _PytestParamConverter(cst.CSTTransformer):
    ...
    - id=/marks=[...]      -> pyrunner param(a, b, id=..., case_id=...,
...
class _FnPlan:
    ...
    def __init__(self):
        ...
        self.remove_params: set[str] = set()  # params replaced by pyrunner imports
...
class MigrationTransformer(cst.CSTTransformer):
    ...
    def __init__(
        ...
        self.needed_imports: set[str] = set()  # names from pyrunner
    ...
    def visit_Import(self, node: cst.Import) -> None:
        for alias in node.names:
            ...
            if name.split(".")[0] in ("pytest", "pyrunner"):
    ...
    def visit_ImportFrom(self, node: cst.ImportFrom) -> None:
        ...
        if module.split(".")[0] not in ("pytest", "pyrunner"):
            return
        for alias in node.names:
    ...
    def leave_FunctionDef(self, original_node, updated_node):
        ...
        if name.startswith("pytest_"):
            msg = f"pytest hook '{name}' -- no pyrunner equivalent"
            self._todo(
                original_node,
                f"pytest hook '{name}' has no pyrunner equivalent "
                f"(pyrunner is deliberately hook-free; see docs)",
            )
            if _has_todo(updated_node.leading_lines, msg):
                return updated_node
            return updated_node.with_changes(
        ...
        if plan.is_async:
            msg = "async test -- pyrunner is sync-only"
            self._todo(
                original_node,
                f"async def {name} -- pyrunner runs sync tests only; "
    ...
    def _plan_function(self, original_node, updated_node) -> _FnPlan | None:
        ...
        for dec in updated_node.decorators:
            ...
            if canonical in (
                "pyrunner.test",
                "pyrunner.fixture",
                "pyrunner.registry.test",
                "pyrunner.registry.fixture",
                "pyrunner.core.registry.test",
                "pyrunner.core.registry.fixture",
            ):
        ...
        # `from pyrunner import parametrize` inserted) even though the
        ...
        for arg in args:
            ...
            if key == "scope" or (key is None and not plan.fixture_args):
                ...
                if scope in _SCOPE_MAP:
                    ...
                    plan.todos.append(
                        'fixture scope="class" -> "module" (no class scope in pyrunner) -- verify'
                    )
                elif scope == "package":
                ...
                plan.todos.append(
                    "fixture ids=... not supported -- pyrunner "
                    "derives the id suffix from the value itself"
                )
            elif key == "name":
                plan.todos.append(
                    f"fixture name={src} alias not supported -- "
                    f"pyrunner registers by function name; rename "
                    f"the function or update references"
                )
            else:
                plan.todos.append(
                    f"fixture argument '{key}={src}' has no pyrunner equivalent -- dropped"
                )
    ...
    def _mark_parametrize(self, args, plan, fn_node, dec):
        ...
        if ids is not None:
            plan.todos.append(
                "parametrize ids=... dropped -- pyrunner derives the test id suffix from the values"
            )
    ...
    def _mark_xfail(self, args, plan, fn_node, dec):
        ...
        if raises is not None:
            plan.todos.append(
                f"xfail(raises={_code(raises)}) -- pyrunner's "
        ...
        # pytest xfail defaults to strict=False; pyrunner fail() to strict=True.
    ...
    def _mark_flaky(self, args, plan, fn_node, dec):
        ...
        if reruns is not None:
            ...
            if any(a.keyword and a.keyword.value == "reruns_delay" for a in args):
                plan.todos.append(
                    "flaky reruns_delay dropped -- pyrunner retries immediately, in-process"
                )
            return
        # Bare `@pytest.mark.flaky`, or the `flaky` package's own
        # max_runs=/min_passes= API -- neither matches pyrunner's
    ...
    def _mark_asyncio(self, args, plan, fn_node, dec):
        plan.todos.append(
            "@pytest.mark.asyncio -- pyrunner is sync-only; "
    ...
    def _plan_signature(self, original_node, updated_node, plan: _FnPlan):
        ...
        for pname in param_names:
            if pname in PROPERTY_FIXTURES and not self.index.fixture_defined(pname):
                ...
                # param, import the pyrunner equivalent instead. Call
                ...
                plan.todos.append(
                    f"builtin fixture '{pname}' has no pyrunner "
                ...
                plan.todos.append(
                    ...
                    f"pyrunner.playwright.playwright_fixtures.configure() / --browser CLI "
                    f"flag instead"
                )
    ...
    def _plan_request_usage(self, updated_node, plan: _FnPlan):
        ...
        for name in sorted(attrs):
            plan.todos.append(
                f"request.{name} is not supported (pyrunner's FixtureRequest only carries .param)"
            )
        # A test whose body no longer touches `request` at all (typically
        # because every request.node.add_marker(...) was rewritten to
        # record_property above) must not keep the parameter: pyrunner
    ...
    def leave_Call(self, original_node, updated_node):
        ...
        # pyrunner -- rename the call itself; the signature param is
    ...
    def _rewrite_add_marker_statement(self, original_node, updated_node):
        ...
        pyrunner has no runtime marker API; record_property lands the value
        ...
        if not convertible:
            msg = (
                "request.node.add_marker has no pyrunner equivalent -- "
    ...
    def _rewrite_call_statement(self, original_node, updated_node):
        ...
        if canonical == "pytest.skip":
            if any(a.keyword and a.keyword.value == "allow_module_level" for a in call.args):
                # pyrunner's skip() raises AT IMPORT TIME, unlike
                ...
                msg = (
                    "pytest.skip(allow_module_level=True) has no pyrunner equivalent "
                    "(pyrunner's skip() raises at import time, breaking module "
