...
class param:
    """One @parametrize entry with per-entry metadata -- pyrunner's
...
def parametrize(arg_names: str | tuple[str, ...] | list[str], arg_values: list[Any]):
    ...
    def decorator(func):
        if getattr(func, "_pyrunner_registered", False):
            raise TypeError(
...
def _stamp_item(func, item):
    ...
    items = getattr(func, "_pyrunner_items", None)
    if items is None:
        items = []
        func._pyrunner_items = items
    items.append(item)
...
def test(
    ...
    def decorator(func):
        ...
        func._pyrunner_registered = True
...
def test_class(
    ...
    def decorator(cls):
        if getattr(cls, "_pyrunner_test_class", False):
            raise TypeError(
        ...
        for attr_name, attr in cls.__dict__.items():
            if isinstance(attr, staticmethod):
                # @test stamps _pyrunner_items on the plain function it
                # wraps; a staticmethod object doesn't forward arbitrary
                # attribute lookups to that function, so
                # getattr(attr, "_pyrunner_items", None) below would
                ...
                if getattr(underlying, "_pyrunner_items", None):
                    raise TypeError(
            ...
            items = getattr(attr, "_pyrunner_items", None)
            if not items:
                continue
            found_any = True
            for item in items:
