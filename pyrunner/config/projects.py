...
CLI flag > project config > base pyrunner.toml > built-in default.
...
No [pyrunner.projects.*] config at all = zero behavior change: no
...
def load_projects(config: dict) -> dict[str, ProjectConfig]:
    """Returns {} if `[pyrunner.projects]` is absent entirely. Raises
    ...
    for name, entry in raw.items():
        ...
        if not tests_dir:
            raise ValueError(f"[pyrunner.projects.{name}] is missing required 'tests_dir'.")
        ...
        if num_workers is not None:
            try:
                ...
                raise ValueError(f"[pyrunner.projects.{name}] num_workers: {e}") from None
        fully_parallel = entry.get("fully_parallel")
        if fully_parallel is not None and not isinstance(fully_parallel, bool):
            raise ValueError(
                f"[pyrunner.projects.{name}] fully_parallel: expected true/false, "
        ...
        if timeout is not None and (
            ...
            raise ValueError(
                f"[pyrunner.projects.{name}] timeout: expected a positive number, got {timeout!r}"
            )
...
def run_projects(
    ...
    *,
    ...
    the same way -- LPT sharding (pyrunner/sharding.py) is scoped by
