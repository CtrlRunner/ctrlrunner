...
pyrunner.toml -- no per-project fixture code needed. Mirrors Playwright
...
def _capture_trace(context_value, prefix, outcome):
    ...
    if not getattr(context_value, "_pyrunner_tracing_active", False):
        return None
    ...
    if mode == "retain-on-failure" and outcome != "failed":
        ...
        context_value._pyrunner_tracing_active = False
        return None
...
def context(browser):
    ...
    ctx._pyrunner_tracing_active = start_tracing
    if start_tracing:
        ctx.tracing.start(screenshots=True, snapshots=True)

    yield ctx

    if getattr(ctx, "_pyrunner_tracing_active", False):
        with contextlib.suppress(Exception):
            ctx.tracing.stop()
    ctx.close()
