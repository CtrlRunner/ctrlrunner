...
`pyrunner flaky-report` output -- never fully automatic. A quarantined
...
def resolve_quarantine_config(config: dict) -> QuarantineConfig | None:
    """Returns None if [pyrunner.quarantine] is absent entirely -- zero
    ...
    if not isinstance(test_ids, (list, tuple)):
        raise ValueError(
            f"[pyrunner.quarantine].test_ids must be a list of test id strings, got {test_ids!r}"
        )
    reason = section.get("reason")
    if reason is not None and not isinstance(reason, str):
        raise ValueError(f"[pyrunner.quarantine].reason must be a string, got {reason!r}")
