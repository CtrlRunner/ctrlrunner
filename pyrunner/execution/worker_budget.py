...
[pyrunner] in pyrunner.toml, [pyrunner.projects.<name>], the UI):
...
class WorkerConstraint:
    ...
    group: str  # config: raw [pyrunner.workers] key; decorator: "relpath.py::ClassName"
    count: int  # >= 1
...
class WorkerConstraintSpec:
    """One parsed [pyrunner.workers] entry, before matching against tests."""
...
def load_worker_constraints(config: dict) -> list[WorkerConstraintSpec]:
    """Parses the [pyrunner.workers] table out of an already-loaded
    [pyrunner] config dict (load_config returns just that inner table,
    so like [pyrunner.grouping], a bare top-level [workers] table is
    silently invisible -- it must be written as [pyrunner.workers]).
    ...
    if not isinstance(table, dict):
        raise ValueError(
            f"[pyrunner.workers] must be a table of 'path' or 'path::ClassName' keys, got {table!r}"
        )
    ...
    for order, (raw_key, value) in enumerate(table.items()):
        ...
        if not path_pattern or (sep and not class_name):
            raise ValueError(
                f"[pyrunner.workers] key {key!r} is malformed -- expected "
                f"'path' or 'path::ClassName'"
            )

        if isinstance(value, bool):
            raise ValueError(
                f"[pyrunner.workers] {key!r}: count must be an integer >= 1, got {value!r}"
            )
        if isinstance(value, int):
            ...
            if unknown:
                raise ValueError(
                    f"[pyrunner.workers] {key!r}: unknown key(s) "
                    f"{', '.join(sorted(unknown))} -- allowed: count, mode"
                )
            if "count" not in value:
                raise ValueError(f"[pyrunner.workers] {key!r}: missing required 'count'")
            count = value.get("count")
            mode_value = value.get("mode", "cap")
            if isinstance(count, bool) or not isinstance(count, int):
                raise ValueError(
                    f"[pyrunner.workers] {key!r}: count must be an integer >= 1, got {count!r}"
                )
            if mode_value not in ("cap", "dedicated"):
                raise ValueError(
                    f"[pyrunner.workers] {key!r}: mode must be 'cap' or 'dedicated', "
                    f"got {mode_value!r}"
                )
            mode = "dedicated" if mode_value == "dedicated" else "cap"
        else:
            raise ValueError(
                f"[pyrunner.workers] {key!r}: value must be an integer >= 1 or a "
...
def _normalized_path(source_path) -> str:
    """Posix path used for [pyrunner.workers] matching: relative to cwd
...
def assign_worker_groups(tests, specs: list[WorkerConstraintSpec]):
    ...
    absent). The most specific matching [pyrunner.workers] spec wins;
...
def build_units(tests, constraints_by_id, fully_parallel_default: bool):
    ...
    class-qualified [pyrunner.workers] entry pulls that class's tests
...
def _unit_weights(units, durations) -> list[tuple]:
    ...
    from pyrunner.execution.sharding import duration_weights
...
def group_aware_shard(
    ...
    """
    from pyrunner.execution.sharding import _lpt_shard_weighted
