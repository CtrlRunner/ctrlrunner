...
def discover_conftests(root: str):
    ...
    root may also be a single file (pytest-style `pyrunner tests/test_x.py`)
...
def discover_modules(root: str):
    ...
    (pytest-style `pyrunner tests/test_x.py`), in which case only that
...
class Orchestrator:
    def __init__(
        ...
        # Scoped worker budgets ([pyrunner.workers] specs) and the
        ...
        # True disables display-filtering of pyrunner-internal
        ...
        # Optional: a HistoryStore (pyrunner/history.py) enables LPT
    ...
    def _reconcile_results(self, selected_ids, results_before) -> None:
        ...
        for tid in selected_ids:
            ...
            result = self.reporter.add_result(
                tid,
                "failed",
                "Internal error: test produced no result (pyrunner bug -- please "
