...
class HistoryStore:
    ...
    def _open(self, db_path: str) -> None:
        ...
        # Multiple pyrunner processes could plausibly touch the same
    ...
    def _migrate_schema(self):
        ...
        COLUMN) is racy across processes -- two pyrunner processes
    ...
    def list_test_ids(self, project: str | None = None) -> list[str]:
        ...
        scoped by project -- section 4.9's `pyrunner flaky-report`
...
def resolve_history_config(config: dict) -> HistoryConfig:
    ...
    if not isinstance(enabled, bool):
        raise ValueError(f"[pyrunner.history].enabled must be a boolean, got {enabled!r}")
    ...
    if db_path is not None and not isinstance(db_path, str):
        raise ValueError(f"[pyrunner.history].db_path must be a string, got {db_path!r}")
    window_runs = section.get("window_runs", 20)
    if not isinstance(window_runs, int) or isinstance(window_runs, bool):
        raise ValueError(f"[pyrunner.history].window_runs must be an integer, got {window_runs!r}")
    if window_runs < 1:
        ...
        raise ValueError(
            f"[pyrunner.history].window_runs must be a positive integer, got {window_runs!r}"
        )
