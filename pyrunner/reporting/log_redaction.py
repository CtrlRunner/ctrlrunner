...
Configuration ([pyrunner.log_redaction] in pyrunner.toml):
...
def resolve_redaction_patterns(config: dict) -> list[re.Pattern] | None:
    """Reads [pyrunner.log_redaction]. Returns compiled patterns (built-in
    ...
    if not isinstance(enabled, bool):
        raise ValueError(
            f"[pyrunner.log_redaction].enabled must be a boolean, got {enabled!r}"
        )
    if not enabled:
        return None
    ...
    if not (isinstance(extra, list) and all(isinstance(p, str) for p in extra)):
        raise ValueError(
            f"[pyrunner.log_redaction].patterns must be a list of strings, got {extra!r}"
        )
    ...
    for raw in (*DEFAULT_REDACTION_PATTERNS, *extra):
        try:
            ...
            raise ValueError(
                f"[pyrunner.log_redaction].patterns: invalid regex {raw!r}: {e}"
            ) from e
    return compiled
